...
::: justify
Para usar o JuMP, precisamos instalar dois componentes: 

1. O pacote **JuMP** (a interface de modelagem). 
2. Um **Solver** (o algoritmo que efetivamente encontra a solução).

Neste guia, utilizaremos o `HiGHS`, um solver de alta performance e código aberto, capaz de lidar com Programação Linear (LP) e Programação Inteira Mista (MIP). Ambos os pacotes podem ser instalados facilmente usando o gerenciador de pacotes do Julia, dessa forma:
:::
...
::: justify
Ao forçar as variáveis a serem inteiras, o lucro máximo pode cair levemente, pois perdemos a flexibilidade dos números fracionários, que poderiam ser utilizados para maximizar o lucro. Essa diferença é chamada de "custo da integridade".
:::
...
optimize!(model_vec)
println("Solução Vetorial (variáveis x_1 e x_2): ", value.(x))
```
...
- Lidar com restrições de números inteiros, para problemas onde as variáveis devem ser inteiras.
- Utilizar a forma vetorial para escalar o problema, sem declarar variáveis individualmente.

Este é apenas o começo. O JuMP também suporta otimização não-linear, cônica, estocástica e até otimização robusta, tornando-o uma das ferramentas mais versáteis disponíveis para Cientistas de Dados e Pesquisadores Operacionais atualmente, e agora você já sabe como utilizá-lo. Pratique com outros problemas clássicos, como o Problema do Caixeiro Viajante (TSP) ou o Problema de Alocação de Recursos, e explore as inúmeras possibilidades que o JuMP oferece para modelar e resolver problemas complexos do mundo real.
:::
