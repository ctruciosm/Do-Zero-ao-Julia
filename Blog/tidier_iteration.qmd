...
description: |
  Saudades do pacote purrr do R? Conheça o TidierIteration.jl, uma implementação em Julia que traz map, walk, flatten e outras ferramentas funcionais para seus pipelines de dados.
categories:
  - Julia
  - Data Science
  - Tidyverse
...
Embora Julia já possua ferramentas nativas poderosas, como *broadcasting* (`f.(x)`) e compreensões de lista, às vezes sentimos falta de uma sintaxe fluida orientada a *pipelines* (`|>`).
...
::: {.justify}
O **TidierIteration.jl** é uma implementação em `Julia` inspirada no `purrr`. Ele visa simplificar a aplicação de funções a *arrays*, *dicionários* e outras estruturas, além de oferecer helpers como `possibly`, `quietly` e funções de achatamento (`flatten`) úteis para lidar com dados aninhados, como *JSON*.
:::
...
::: {.justify}
`Julia` já possui `map(f, x)`. O TidierIteration introduz o `map_tidy`, que recebe os dados como primeiro argumento. Isso torna a leitura do código mais consistente em pipelines.
...
Se tentássemos usar `imap` no lugar de `iwalk`, perderíamos nossos dados. A função `display` retorna `nothing`, e o `map` passaria esse retorno adiante, substituindo os gráficos por uma sequência de valores nulos.
...
::: {.justify}
Como resultado, `resultado_quebrado` vira um vetor de `nothing` e o pipeline perde os gráficos originais. Com `walk`, os dados originais (`meus_plots`) são preservados e passam adiante.
:::

# Domando JSONs com `flatten`

::: {.justify}
Quem trabalha com APIs sabe o desafio de lidar com listas de listas de dicionários. As funções `flatten_*` ajudam a resolver esse problema.
:::
