...
Y = X1 .+ X2.*X3 # Criação do vetor resposta
```
```{julia}
...
::: justify
Ao trabalhar com modelos de aprendizado de máquina, é de interesse saber o valor ideal dos nossos hiperparâmetros em questão. No caso da floresta aleatória, existem 7 parâmetros que podemos especificar. Felizmente, existe uma maneira muito eficaz de comparar diversas combinações de hiperparâmetros e escolher a melhor por meio de validação cruzada.
...
Com isso, para especificar os valores de cada hiperparâmetro a serem testados, é necessário utilizar a função `range`, determinando os valores mínimo e o máximo a serem considerados. Já para utilizar validação cruzada do tipo k-fold, é necessário utilizar a função `CV`, especificando por meio do argumento `nfolds`, o número de dobras a serem consideradas. Para o exemplo, iremos utilizar 5 folds e a acurácia, por se tratar de classificações. 
:::
```{julia}
Random.seed!(123)
...
floresta_escolher_k = TunedModel(model=floresta_classificacao,
                               ...
                               range=[valores_n_trees, valores_max_depth],
                               measure=accuracy)
```
::: justify
...
Neste tutorial, mostramos como podemos implementar a floresta aleatória em Julia para regressão e classificação de forma eficiente, enfatizando etapas cruciais como divisão dos dados em treino e teste, avaliação de métricas de erro, aplicação de predições e escolha do conjunto ideal de hiperparâmetros por validação cruzada, continue explorando e aplicando essa poderosa técnica de aprendizado de máquina em seus projetos!
:::
