...
::: {.justify}
Temos a matriz $A$ e queremos encontrar o autovalor e o autovetor associados a um valor de referência (o shift) $\rho = 8$.
...
::: {.justify}
Vamos rodar o algoritmo por 12 iterações. A cada passo, multiplicamos $B$ pelo vetor atual, normalizamos e calculamos o autovalor estimado por meio do quociente de Rayleigh.
:::
...
::: {.justify}
Agora vem a parte interessante. A teoria nos diz que a convergência é linear e que a taxa depende da distância entre o nosso shift $\rho$ e os autovalores da matriz.

Vamos calcular uma razão de convergência observada: $\text{Razão} = \frac{\|q_{j+1} - v\|}{\|q_j - v\|}$, onde $v$ é a nossa melhor estimativa do autovetor (o resultado da última iteração).
:::
...
::: {.justify}
Será que essa taxa observada faz sentido? Vamos calcular os autovalores exatos com Julia (`eigvals`) e aplicar a fórmula da taxa teórica:
...
Isso demonstra o poder de prever o comportamento de algoritmos numéricos. Se a razão for pequena, a convergência é rápida; se estiver próxima de 1, o algoritmo converge mais lentamente. Na iteração inversa, controlamos essa velocidade escolhendo um shift $\rho$ adequado, de modo que o autovalor desejado fique próximo do valor de referência.
:::
