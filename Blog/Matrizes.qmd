...
::: {.justify}
Matrizes são estruturas matemáticas em formato tabular que se organizam em linhas e colunas, e possuem operações bem definidas como adição, subtração, multiplicação e transposição. De forma simplificada, podem ser entendidas como uma extensão bidimensional dos vetores. Elas são amplamente utilizadas para armazenar dados e desempenham um papel fundamental em áreas como álgebra linear e métodos de *Machine Learning*. 

Neste post, você aprenderá a criar matrizes, acessar elementos, realizar operações e explorar suas principais propriedades. Além disso, será mostrado brevemente como utilizar o pacote `LinearAlgebra.jl` para algumas funcionalidades relacionadas à álgebra linear.
:::

## Criando Matrizes

::: {.justify}
Em *Julia*, a notação tradicional de matrizes utiliza colchetes `[ ]`. Dentro deles, o `espaço` é usado para separar as colunas, o ponto e vírgula (`;`) separa as linhas e o triplo ponto e vírgula (`;;;`) separa os "andares" — representando a terceira dimensão e criando um array 3D a partir de matrizes 2D.
:::
...
-   `ones(m, n)`: gera uma matriz m × n preenchida com uns.

-   `rand(m, n)`: cria uma matriz m × n com valores aleatórios entre 0 e 1.

-   `reshape(v, m, n)`: transforma um vetor `v` ou matriz existente em uma nova forma m × n.

-   `Matrix(...)`: converte estruturas em matriz; por exemplo, `Matrix(I, m, m)` cria uma matriz identidade m x m.
:::
...
::: {.justify}
Assim como em R, a indexação em Julia é *1-based*, ou seja, a contagem começa a partir de 1 — diferente do Python, que utiliza *0-based indexing*.
...
::: {.justify}
*Julia* permite realizar operações matemáticas entre matrizes de forma simples e direta, desde que suas dimensões sejam compatíveis — ou seja, a relação entre os números de linhas e colunas precisa ser adequada à operação desejada.
:::
...
-   `*`: Multiplicação matricial (produto linear), usada para combinar matrizes ou multiplicar uma matriz por um escalar.

-   `.*`: Multiplicação elemento a elemento, onde cada elemento da primeira matriz é multiplicado pelo elemento correspondente da segunda matriz.
:::
...
### Transposição
::: {.justify}
Também é possível transpor uma matriz, ou seja, trocar suas linhas por colunas. A função `transpose()` realiza essa troca independentemente do tipo de dado presente na matriz. Já o operador `'` produz o mesmo resultado para matrizes com números reais, mas, no caso de números complexos, ele aplica a conjugação complexa, invertendo o sinal da parte imaginária.
:::

```{julia}
# Transposição explícita 
transpose(G)
# Atalho para transposição
G'
```
