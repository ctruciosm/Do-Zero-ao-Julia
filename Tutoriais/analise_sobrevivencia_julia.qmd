...
Random.seed!(42) # Tornando o código reprodutível pois os dados gerados serão os mesmos a cada execução
```
...
```{julia}
# Número de observações (indivíduos simulados)
n = 400
...
# Taxa de risco basal (hazard baseline)
λ0 = 0.08

# Hazard Ratio do tratamento
hr_treat = 0.6

# Taxa de risco individual λ para cada pessoa
# "Tratamento":
#   λ = λ0 * 0.6
# "Controle":
#   λ = λ0 * 1.0
...
# Tempo observado no estudo
time = min.(time_true, censor_time)

df = DataFrame(
    time = time,     # tempo observado
    event = event,   # indicador de evento/censura
    group = group,   # grupo experimental
    age = age        # idade
)

# Objeto de Survival.jl para análise de sobrevivência
...
bar([sum(df.event), sum(.!df.event)],
    ...
    title = "Eventos observados vs censura",
    legend=:top)
```
...
# Comparação visual entre grupos (Nelson–Aalen)
