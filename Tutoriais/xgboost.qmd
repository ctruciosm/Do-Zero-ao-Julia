...
```
::: justify
Também podemos calcular métricas de erro para o nosso modelo, por exemplo, podemos calcular a acurácia com que as predições estão corretas utilizando a função `accuracy`.
:::
```{julia}
...
xgboost_escolher_k = TunedModel(model=xgboost_classificacao,
                               ...
                               range=[valores_eta, valores_max_depth],
                               measure=accuracy)
```
::: justify
