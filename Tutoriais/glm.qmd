...
::: justify
Para realizar a estimativa de um Modelo Linear Generalizado com o pacote `GLM.jl`, utilizamos a função `glm()`, e existem duas maneiras de realizar esse procedimento:
...
As opções de família de distribuições disponíveis são:
...
::: justify
Ao utilizar variáveis categóricas como covariáveis, o pacote cria automaticamente variáveis indicadoras (dummy variables) e, por padrão, utiliza a categoria de referência, escolhendo o primeiro nível da variável como base para comparação. Assim, os coeficientes estimados para as demais categorias representam diferenças em relação a essa categoria de referência.
...
```
::: justify
Ajustando o modelo sem especificar nada, automaticamente foi utilizada a parametrização de Casela de Referência, ou Categoria de Referência em que "A" foi considerado a referência.
:::
```{julia}
...
```
::: justify
Também podemos utilizar a parametrização de Efeito Diferencial usando a mesma função `Dict()`, mas agora com o argumento `EffectsCoding()`. Assim, aplicando essa parametrização no nosso exemplo, temos:
:::
```{julia}
...
::: justify
A partir do objeto de um modelo, podemos extrair diversas informações que podem ser do nosso interesse. Por exemplo, para extrair os valores dos coeficientes podemos utilizar a função `coef()`.
:::
```{julia}
coef(modelo_exemplo)
```
...
```
::: justify
Além disso, a depender da distribuição, também temos o parâmetro de dispersão, no caso da Normal, $\sigma^2$, por exemplo. Para obter o seu valor estimado, utilizamos a função `dispersion()`.
:::
```{julia}
...
::: justify
Com um modelo ajustado, existem diversas funções que podemos aplicar nele para obter métricas de interesse, sendo algumas delas:
...
::: justify
Agora você já conhece um pouco de como trabalhar com Modelos Lineares Generalizados em Julia. Com o poderoso pacote `GLM.jl` e a eficiência computacional da linguagem, é possível realizar análises estatísticas das mais variadas naturezas com rapidez e precisão.

Combine essa ferramenta com outras ferramentas abordadas em nossos outros tutoriais para tornar suas análises em Julia cada vez mais robustas, integradas e profissionais. Continue se aprofundando e explorando o mundo da modelagem estatística com Julia!
:::
