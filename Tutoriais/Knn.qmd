...
```
::: justify
Para um k-NN do tipo regressão, a variável resposta deve ser numérica. Vamos criar o vetor `Y` com a variável resposta e o DataFrame `X` com as covariáveis:
:::
```{julia}
#| output: false
Random.seed!(123)
...
```
::: justify
Agora, vamos separar os dados em treino e teste utilizando a função `partition`. No caso, vamos deixar 80% dos dados para treino e os demais para teste.
:::
```{julia}
#| output: false
Random.seed!(123)
...
```
::: justify
Para regressão, utilizamos `KNNRegressor` e especificamos o número de vizinhos desejados por meio do argumento `K`:
:::
```{julia}
knn_regressao = KNNRegressor(K = 5)
```
::: justify
Entretanto, não basta apenas ajustar o modelo. Como ele é baseado na norma euclidiana, é necessário padronizar as covariáveis, a fim de a escala dos dados não influenciar na eficácia do modelo. Para isso, vamos utilizar a função `Pipeline` para especificar um pipeline que padroniza os dados utilizando `Standardizer` e após isso ajusta o modelo.  
:::
```{julia}
...
::: justify
Ao trabalhar com modelos de aprendizado de máquina, é de interesse saber o valor ideal dos nossos hiperparâmetros em questão. No caso do k-NN, é necessário determinar qual o valor ideal de `K`. Felizmente, existe uma maneira muito eficaz de comparar diversos valores de `K` e escolher o melhor por meio de validação cruzada.

Para realizar esse procedimento de escolher automaticamente o melhor valor de `K` por meio de uma busca em grade (grid search) avaliada com validação cruzada, é necessário utilizar a função `TunedModel`, que possui como argumentos:
...
Com isso, para especificar os valores de `K` a serem testados, é necessário utilizar a função `range`, determinando os valores mínimo e o máximo a serem considerados. Já para utilizar validação cruzada do tipo k-fold, é necessário utilizar a função `CV`, especificando em `nfolds` o número de dobras a serem consideradas. Para o exemplo, iremos utilizar 5 folds e a acurácia por se tratar de classificações. 
:::
```{julia}
#| output: false
Random.seed!(123)
...
```
::: justify
Assim como no modelo anterior, é necessário padronizar as covariáveis, para isso, criaremos um pipeline da mesma forma:
:::
...
```
::: justify
Como o modelo ajustado faz parte de um Pipeline, os parâmetros ajustados ficam organizados por etapa do pipeline. Assim, acessamos primeiro o componente modelo e, em seguida, o `best_model`. No caso, o melhor modelo variando o número de vizinhos de 1 a 10, foi: 
:::
```{julia}
melhor_knn = fitted_params(machine_escolher_k).modelo.best_model
```

## Conclusão

::: justify
O k-NN é um algoritmo simples de compreender, mas bastante eficiente quando utilizado corretamente. Como seu desempenho depende diretamente da medida de distância entre as observações, etapas como a padronização das covariáveis e a escolha adequada do número de vizinhos são fundamentais.

Neste tutorial, vimos como utilizar o k-NN em Julia tanto para regressão quanto para classificação, desde a preparação dos dados até a avaliação do modelo. Também mostramos como empregar validação cruzada para selecionar automaticamente o melhor valor de `K`, tornando o processo de modelagem mais robusto e confiável.

Apesar de sua simplicidade, o k-NN continua sendo uma excelente alternativa para diversos problemas de aprendizado supervisionado, servindo tanto como modelo inicial de comparação quanto como uma solução competitiva em aplicações práticas.
:::
